mod_continuous_ui <- function(id) {
  ns <- NS(id)
  tagList(
    h3("Continuous Outcome: Two-Sample Parallel Design"),
    fluidRow(
      box(
        title = "Inputs",
        width = 6,
        ...
        conditionalPanel(
          ...
          numericInput(ns("margin"), "Margin (for NI/Equivalence)", value = NA, step = 0.1)
        ),
    ...
    fluidRow(
      box(
        ...
        collapsible = TRUE,
        collapsed = TRUE,
        p("This module calculates sample size or achieved power for a continuous outcome using a two-sample parallel design."),
        tags$ul(
          tags$li("Enter the expected mean difference (Δ) and standard deviation (σ)."),
...
#         if (is.null(input$delta) || is.null(input$sd))
#           return("Please provide both Δ (mean difference) and σ (standard deviation).")
#
...
#         cat("=== Continuous Outcome: Two-Sample Parallel Design ===\n\n")
#
#         if (input$mode == "n") {
